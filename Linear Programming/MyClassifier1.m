classdef MyClassifier1 < handle
    ...
    methods (Access = public)
        ...
        function obj = train(obj,trainData,trainLabel)
            ...
            % This Classifier uses "One VS One" SVM to solve
            % multiclassification problem. 
            % Mention that W has size of(# of features+1, # of hyperplanes)
            % w has element of(i;j). i represents +1 labels, j represents
            % -1 labels.
            for i=(1:obj.K-1)
                for j=(i+1:obj.K)
                    newTrainLabel = [];
        ...
        function [testResults] = classify(obj,testData)
            ...
            for index =(1:num_of_points)
               ...
               for hyperplane = (1:num_of_hyperplanes)
                    if (testData(:,index))'*obj.W(:,hyperplane)>=0
                        counts(obj.w(1,hyperplane)) = counts(obj.w(1,hyperplane))+1;
                    else
                        counts(obj.w(2,hyperplane)) = counts(obj.w(2,hyperplane))+1;
                    end
               end
