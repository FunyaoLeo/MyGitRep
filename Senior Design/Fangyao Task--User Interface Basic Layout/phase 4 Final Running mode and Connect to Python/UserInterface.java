...
public class UserInterface {
	...
	public static void main(String[] args) {
		UserInterface setting = new UserInterface();
		//setting.go();
	}
	/*******************/
		...
		class runningModeListener implements ActionListener{
			public void actionPerformed(ActionEvent event){
				if((width != -1)&&(length != -1)&&(xList.size()!=0)&&(yList.size()!=0)){
					try{
						frame.dispose();
						new RunningMode().go(mapName,beaconNum,width,length,xList,yList);
						comm new1 = new comm();
						new Thread(new1).start();
					}
					catch(Exception e) {
					...
					JOptionPane.showMessageDialog(frame,"Please input valid parameters for all blanks first!");
			}
		}
