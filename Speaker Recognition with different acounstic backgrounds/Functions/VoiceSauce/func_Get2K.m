function [H2K, F2K, isComplete] = func_Get2K(y, Fs, F0, handles, variables, textgridfile)
% [H2K, isComplete] = func_Get2K(y, Fs, F0, handles, variables, textgridfile)
...
% Output: H2K - harmonic 2K vector
%         F2K - frequency of H2K vector 
...
if (nargin == 5)
    ...
    for k=1:length(F0)
        ...
        [h2k, f2k] = func_GetHarmonics(yseg, 2000, F0_curr, Fs);
...
%--------------------------------------------------------------------------
function [h,fh]=func_GetHarmonics(data,f2_est,f0_est,Fs)
...
df = 0.5;     % search around f2_est in steps of df (in Hz)
df_range = round(f0_est*df); % search range (in Hz)

f_min = f2_est - df_range;
f_max = f2_est + df_range;

f = @(x)func_EstMaxVal(x, data, Fs);
options = optimset('Display', 'off');
warning off;
%[x, val, exitflag, output] = fmincon(f, f_est, [], [], [], [], f_min, f_max, [], options);
[x, val, exitflag, output] = fminsearchbnd(f, f2_est, f_min, f_max, options);  
