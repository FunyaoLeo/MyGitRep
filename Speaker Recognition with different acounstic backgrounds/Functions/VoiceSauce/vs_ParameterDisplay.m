...
for k=1:length(VSData.vars.PD_paramselection)
    if (VSData.vars.PD_paramselection(k) <= length(paramlist))
        C = textscan(paramlist{VSData.vars.PD_paramselection(k)}, '%s %s', 'delimiter', '(');
        param = C{2}{1}(1:end-1);
        labels{k} = param;

        t = [1:length(mdata.(param))] * mdata.frameshift;
        if strcmp(labels{k},'soe')
            stem(subp2, t, mdata.(param), VSData.vars.PD_plottype{mod(k-1, VSData.vars.PD_maxplots)+1},'x');
        elseif strcmp(labels{k},'epoch')
            stem(subp2, t, mdata.(param), VSData.vars.PD_plottype{mod(k-1, VSData.vars.PD_maxplots)+1},'x');
        else
