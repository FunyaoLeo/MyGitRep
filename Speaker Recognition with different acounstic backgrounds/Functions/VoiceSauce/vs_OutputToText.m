...
% Last Modified by GUIDE v2.5 17-Jun-2015 17:11:49
...
VSData.vars.OT_Epoch = [vars.OT_outputdir vars.dirdelimiter vars.OT_Epochfilename];
...
set(handles.checkbox_Parameters_includeAlgorithmData, 'Value', vars.OT_outputAlgorithmMetadata);
...
set(handles.edit_OutputOptions_Epoch, 'String', vars.OT_Epoch);
...
if (vars.OT_multipleFiles == 1)
    ...
    set(handles.edit_OutputOptions_Epoch, 'Enable', 'On');
    ...
    set(handles.pushbutton_OO_EpochBrowse', 'Enable', 'On');
...
VSData.vars.OT_Epoch = [VSData.vars.OT_outputdir VSData.vars.OT_Epochfilename];
...
set(handles.edit_OutputOptions_Epoch, 'String', VSData.vars.OT_Epoch);
...
% handles    structure with handles and user data (see GUIDATA)
VSData = guidata(handles.VSHandle);
[newfile newpath] = uiputfile({'*.txt', '*.txt'}, 'Select Output File', VSData.vars.OT_Single);
if (ischar(newfile) == 1)
...
% handles    structure with handles and user data (see GUIDATA)
VSData = guidata(handles.VSHandle);
[newfile, newpath] = uiputfile({'*.txt', '*.txt'}, 'Select Output File', VSData.vars.OT_F0CPPE);
if (ischar(newfile) == 1)
...
% handles    structure with handles and user data (see GUIDATA)
VSData = guidata(handles.VSHandle);
[newfile, newpath] = uiputfile({'*.txt', '*.txt'}, 'Select Output File', VSData.vars.OT_Formants);
if (ischar(newfile) == 1)
...
% handles    structure with handles and user data (see GUIDATA)
VSData = guidata(handles.VSHandle);
[newfile, newpath] = uiputfile({'*.txt', '*.txt'}, 'Select Output File', VSData.vars.OT_HA);
if (ischar(newfile) == 1)
...
% handles    structure with handles and user data (see GUIDATA)
VSData = guidata(handles.VSHandle);
[newfile, newpath] = uiputfile({'*.txt', '*.txt'}, 'Select Output File', VSData.vars.OT_HxHx);
if (ischar(newfile) == 1)
...
% handles    structure with handles and user data (see GUIDATA)
VSData = guidata(handles.VSHandle);
[newfile, newpath] = uiputfile({'*.txt', '*.txt'}, 'Select Output File', VSData.vars.OT_HxAx);
if (ischar(newfile) == 1)
...
% --- Executes on button press in pushbutton_OO_EpochBrowse.
function pushbutton_OO_EpochBrowse_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_OO_EpochBrowse (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
VSData = guidata(handles.VSHandle);
[newfile, newpath] = uiputfile({'*.txt', '*.txt'}, 'Select Output File', VSData.vars.OT_Epoch);
if (ischar(newfile) == 1)
    set(handles.edit_OutputOptions_Epoch, 'String', [newpath newfile]);
    VSData.vars.OT_Epoch = [newpath newfile];
    guidata(handles.VSHandle, VSData);
end
...
% handles    structure with handles and user data (see GUIDATA)
VSData = guidata(handles.VSHandle);
[newfile, newpath] = uiputfile({'*.txt', '*.txt'}, 'Select Output File', VSData.vars.OT_EGG);
if (ischar(newfile) == 1)
...
switch (get(eventdata.NewValue, 'Tag'))
    case 'radiobutton_Singlefile'
        ...
        set(handles.edit_OutputOptions_Epoch, 'Enable', 'Off');
        ...
        set(handles.pushbutton_OO_EpochBrowse, 'Enable', 'Off');
...
% check output files
fids = zeros(7, 1);  % these store the fids of the open files  
if (get(handles.radiobutton_Singlefile, 'Value') == 1) % single file
    ...
    fids = [fid fid fid fid fid fid fid];
    ...
    fid6 = fopen(VSData.vars.OT_Epoch, 'wt');
    ...
    if (fid6 == -1)
        msgbox('Error: Unable to open epoch file for output.', 'Error', 'error', 'modal');
        return;
    end
    ...
    fids = [fid1 fid2 fid3 fid4 fid5 fid6 fidEGG];
...
for k=1:length(matfilelist)
    ...
    if (proceedEGG)
        ...
        if (isempty(EGGData))
            proceedEGG = 0;
            uiwait(msgbox(['Unable to read ' EGGfile ' - skipping file.'], 'Warning - corrupt file', 'warn', 'modal'));
        end
    end
    ...
    if (VSData.vars.OT_noSegments == 1)
        ...
        for n=1:length(start)
            ...
            sstop(sstop > (maxlen / frameshift)) = maxlen / frameshift; % guard against going over data length
            ...
            for s=sstart:sstop
                ...
                for m=1:length(uniquefids)
                    ...
                    if (VSData.vars.OT_outputAlgorithmMetadata == 1)
                        fprintf(uniquefids(m), '%s%c', mdata.AFMTalgorithm, delimiter);
                        fprintf(uniquefids(m), '%s%c', mdata.H2KFMTalgorithm, delimiter);
                        fprintf(uniquefids(m), '%s%c', mdata.HF0algorithm, delimiter);
                        fprintf(uniquefids(m), '%s%c', mdata.BandwidthMethod, delimiter);
                        fprintf(uniquefids(m), '%d%c', mdata.frameshift, delimiter);
                        fprintf(uniquefids(m), '%f%c', mdata.preemphasis, delimiter);
                        fprintf(uniquefids(m), '%d%c', mdata.windowsize, delimiter);
                    end
...
for k=1:length(fids)
    ...
    if (VSData.vars.OT_outputAlgorithmMetadata == 1)
        fprintf(fids(k), 'AFMTalgorithm%c', delimiter);
        fprintf(fids(k), 'H2KFMTalgorithm%c', delimiter);
        fprintf(fids(k), 'HF0algorithm%c', delimiter);
        fprintf(fids(k), 'BandwidthMethod%c', delimiter);
        fprintf(fids(k), 'frameshift%c', delimiter);
        fprintf(fids(k), 'preemphasis%c', delimiter);
        fprintf(fids(k), 'windowsize%c', delimiter);
    end
...
if (length(fids)==1)
    if (VSData.vars.OT_includeEGG)
        fids = [fids fids fids fids fids fids fids];
    else
        fids = [fids fids fids fids fids fids -1];
    end
end
...
% --- Executes on button press in checkbox_Parameters_includeAlgorithmData.
function checkbox_Parameters_includeAlgorithmData_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_Parameters_includeAlgorithmData (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox_Parameters_includeAlgorithmData
VSData = guidata(handles.VSHandle);
VSData.vars.OT_outputAlgorithmMetadata = get(hObject, 'Value');
guidata(handles.VSHandle, VSData);



function edit_OutputOptions_Epoch_Callback(hObject, eventdata, handles)
% hObject    handle to edit_OutputOptions_Epoch (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_OutputOptions_Epoch as text
%        str2double(get(hObject,'String')) returns contents of edit_OutputOptions_Epoch as a double


% --- Executes during object creation, after setting all properties.
function edit_OutputOptions_Epoch_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_OutputOptions_Epoch (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
